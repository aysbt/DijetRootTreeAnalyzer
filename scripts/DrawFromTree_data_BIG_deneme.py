...
for f in fileNames:
  ...
  #standard
  tree.Project(h_allCuts.GetName(), var,'deltaETAjj<1.3 && QG1stJet>0.5 && QG2ndJet>0.5 && mjj > '+str(minX_mass))
  # blinded 4 TeV
...
if golden:
  chain.Project("h_dat", var,'deltaETAjj<1.3 && QG1stJet>0.5 && QG2ndJet>0.5 && mjj > '+str(minX_mass)+' && run >= '+ str(runMin) + '&& run <= ' + str(runMax) +' && PassJSON==1')
else:
  chain.Project("h_dat", var,'deltaETAjj<1.3 && QG1stJet>0.5 && QG2ndJet>0.5 && mjj > '+str(minX_mass)+' && run >= '+ str(runMin) + ' && run <= ' + str(runMax) )
...
kFactor_BIG = NDAT/NQCD
kFactor =0.906713
print ("kFactor set to %f" % kFactor_BIG)
print("NQCD = "+str(NQCD))
print("NDAT = "+str(NDAT))
print("NSIG = "+str(NSIG))
...
outFile = TFile(outputDir+"histo_data_"+var+"_fromTree_Big_05_deneme.root", "recreate")
outFile.cd()
#scale to data
