...
if golden:
  chain.Project("h_dat", var,'deltaETAjj<2.6 && mjj > '+str(minX_mass)+' && run >= '+ str(runMin) + '&& run <= ' + str(runMax) +' && PassJSON==1')
else:
  chain.Project("h_dat", var,'deltaETAjj<2.6 && mjj > '+str(minX_mass)+' && run >= '+ str(runMin) + ' && run <= ' + str(runMax) )
...
outFile = TFile(outputDir+"histo_data_"+var+"_fromTree_nocut.root", "recreate")
outFile.cd()
#scale to data
